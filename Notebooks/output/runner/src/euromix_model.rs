...
fn deserialize_doses<'de, D>(deserializer: D) -> Result<Vec<(f64, f64)>, D::Error>
where
    D: Deserializer<'de>,
{
    let vec: Vec<Vec<f64>> = Deserialize::deserialize(deserializer)?;
    vec.into_iter()
        .map(|v| {
            if v.len() == 2 {
                Ok((v[0], v[1]))
            } else {
                Err(serde::de::Error::custom("Each dose must be an array of 2 elements [time, amount]"))
            }
        })
        .collect()
}
...
pub struct SimulationParams {
    ...
    #[serde(deserialize_with = "deserialize_doses")]
...
pub fn run_simulation_impl(params: &str) -> String {
    ...
    let f_su = sim_params.f_su;
    let f_se = sim_params.f_se;
    let VBlood = sim_params.VBlood;
    let FBlood = sim_params.FBlood;
    let FFat = sim_params.FFat;
    let FPoor = sim_params.FPoor;
    let FLiver = sim_params.FLiver;
    let FSkin = sim_params.FSkin;
    let FRich = sim_params.FRich;
    let FSkin_e = sim_params.FSkin_e;
    let FSkin_u = sim_params.FSkin_u;
    let Air = sim_params.Air;
    let Urine = sim_params.Urine;
    let Fat = sim_params.Fat;
    let Rich = sim_params.Rich;
    let Liver = sim_params.Liver;
    let Art = sim_params.Art;
    let Ven = sim_params.Ven;
    let Skin_e = sim_params.Skin_e;
    let Skin_u = sim_params.Skin_u;
    let Skin_sc_e = sim_params.Skin_sc_e;
    let Skin_sc_u = sim_params.Skin_sc_u;
    let Poor = sim_params.Poor;
    ...
    let rhs = |y: &diffsol::NalgebraVec<f64>, _p: &diffsol::NalgebraVec<f64>, _t: f64, dy: &mut diffsol::NalgebraVec<f64>| {
        ...
        let x0 = Art.powi(-1);
        let x1 = -QArt*x0;
        let x2 = 1.0/(Fat*PCFat);
        let x3 = QFat*x2;
        let x4 = 1.0*FFat;
        let x5 = 1.0/(PCRich*Rich);
        let x6 = QRich*x5;
        let x7 = 1.0*FRich;
        let x8 = 1.0/(PCPoor*Poor);
        let x9 = QPoor*x8;
        let x10 = 1.0*FPoor;
        ...
        dy[8] = x46*(-x28 + x31*x35);
        dy[9] = x47*(x35*x42 - x39);
    ...
    let jac = |y: &diffsol::NalgebraVec<f64>, _p: &diffsol::NalgebraVec<f64>, _t: f64, v: &diffsol::NalgebraVec<f64>, jv: &mut diffsol::NalgebraVec<f64>| {
        ...
        jv[4] += (1.0*x59) * v[3];
        jv[5] += (-x61) * v[5];
        jv[6] += (-x64*(x33 + x63)) * v[6];
        jv[6] += (x65) * v[8];
        jv[6] += (FSkin_u*x62) * v[10];
        jv[7] += (-x67*(x43 + x66)) * v[7];
        jv[7] += (x68) * v[9];
        ...
        jv[9] += (x66*x67) * v[7];
        jv[9] += (-x68) * v[9];
        jv[10] += (-x62*(FFat + FLiver + FPoor + FRich + FSkin_e + FSkin_u + x50 + x53)) * v[10];
        jv[10] += (x69) * v[11];
        jv[10] += (x70) * v[13];
        jv[11] += (x55) * v[0];
        jv[11] += (x56) * v[1];
